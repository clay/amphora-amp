...
function getMediaMap({ _components: componentList, _layoutData, locals }) {
  const layoutName = _layoutData && _layoutData.name,
    ...
    defaultLinkedFontPath = path.join(assetDir, 'css', '_linked-fonts._default.css'),
    siteLinkedFontPath = path.join(assetDir, 'css', `_linked-fonts.${siteStyleguide}.css`),
    defaultInlinedFontPath = path.join(assetDir, 'css', '_inlined-fonts._default.css'),
    siteInlinedFontPath = path.join(assetDir, 'css', `_inlined-fonts.${siteStyleguide}.css`);
  ...
  componentStyles.push(defaultLinkedFontPath);
  componentStyles.push(siteLinkedFontPath);
  componentStyles.push(defaultInlinedFontPath);
  componentStyles.push(siteInlinedFontPath);
