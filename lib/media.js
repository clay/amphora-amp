  ...
  return getContentsOfFiles(fileArray, directory, filePrefix)
    .then(function (fileContents) {
      ...
      if (util.getByteLength(joined) > 50000) {
        ...
        log('info', `Combined CSS contents in bytes ${util.getByteLength(joined)}`);
      }
...
function getStyles(name, site) {
  ...
  if (!site.styleguide) {
    cssFilePaths = [
      path.join(assetDir, 'css', `${name}_amp.css`),
      path.join(assetDir, 'css', `${name}_amp.${stylesSource}.css`)
    ];
  } else {
    const ampPath = path.join(assetDir, 'css', `${name}_amp.${stylesSource}.css`);

    cssFilePaths = util.fileExists(ampPath) ? [ampPath] : [path.join(assetDir, 'css', `${name}_amp._default.css`)];
  }

  return _.map(_.filter(cssFilePaths, util.fileExists), pathJoin(assetHost, assetPath, assetDir));
}
