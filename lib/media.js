...
function getMediaMap({ _components: componentList, _layoutData, locals }) {
  const layoutName = _layoutData && _layoutData.name,
    ...
    defaultFontPath = path.join(assetDir, 'css', '_linked-fonts._default.css'),
    siteFontPath = path.join(assetDir, 'css', `_linked-fonts.${siteStyleguide}.css`);
  ...
  componentStyles.push(defaultFontPath);
  componentStyles.push(siteFontPath);
