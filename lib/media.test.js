...
describe(_.startCase(filename), function () {
  ...
  describe('getStyles', function () {
    ...
    it('accepts good component with slug (with slug file)', function () {
      util.fileExists.onCall(0).returns(true);
      util.fileExists.onCall(1).returns(true);
    ...
    it('accepts good component with slug (with slug file) with assetDir', function () {
      util.fileExists.withArgs('/foo/css/name_amp.css').returns(true);
      util.fileExists.withArgs('/foo/css/name_amp.foo.css').returns(true);

      expect(fn('name', state.locals.site)).to.deep.equal(['/css/name_amp.css', '/css/name_amp.foo.css']);
    });

    it('uses the MEDIA_DIRECTORY if one is not defined on the site', function () {
      util.fileExists.withArgs(`${process.cwd()}/public/css/name_amp.css`).returns(true);
      util.fileExists.withArgs(`${process.cwd()}/public/css/name_amp.foo.css`).returns(true);
    ...
    it('attempts to find a default style if the amp style variation cannot be found', function () {
      util.fileExists.withArgs('/foo/css/name_amp.bar.css').returns(false);
      util.fileExists.withArgs('/foo/css/name._default.css').returns(true);
