...
jest.mock('clayhandlebars', () => {
  const t = () => {
    const b = {
      ...
      registerPartial: jest.fn().mockImplementation((name, content) => {
        b.partials[name] = content;
      })
    };
  ...
  t.wrapPartial = (componentName) => {
    if (componentName === 'c1') return 'hello, world';
    if (componentName === 'c2') return null;
    return '';
  };
...
describe(_.startCase(filename), () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(amphoraFs, 'getLayouts');
  ...
  describe('init', () => {
    ...
    test(
      ...
      () => {
        amphoraFs.getLayouts.mockReturnValue(['l1']);
